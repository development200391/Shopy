...
const _quickReplies = ['Siap kak', 'Ready ya kak', 'Terima kasih'];

/// Halaman **Ruang Chat** — desain terpilih: **Bold & Colorful**
/// (lihat `design/assets/chat-seller-bold-colorful.png`).
///
/// Realtime cukup polling 5 detik selama layar terbuka + push FCM saat di
...
class _ChatRoomScreenState extends ConsumerState<ChatRoomScreen> {
  ...
  void initState() {
    ...
    ref.read(sellerChatApiServiceProvider).markRead(widget.room.id);
  ...
  Future<void> _load({required bool showSpinner}) async {
    ...
    try {
      final messages = await ref.read(sellerChatApiServiceProvider).getMessages(widget.room.id);
      if (!mounted) return;
      setState(() {
        _messages = messages;
        _loading = false;
      });
      unawaited(ref.read(sellerChatApiServiceProvider).markRead(widget.room.id));
    } catch (_) {
      if (mounted) setState(() => _loading = false);
    }
  }

  Future<void> _send({String? body, String? attachmentUrl, String? productId}) async {
    if ((body == null || body.trim().isEmpty) && attachmentUrl == null && productId == null) return;

    setState(() => _sending = true);
    try {
      await ref
          .read(sellerChatApiServiceProvider)
          .sendMessage(widget.room.id, body: body?.trim(), attachmentUrl: attachmentUrl, productId: productId);
  ...
  Future<void> _pickImage() async {
    ...
    setState(() => _sending = true);
    try {
      final url = await ref.read(sellerApiServiceProvider).uploadFile(picked.path, 'chat');
  ...
  Future<void> _pickProduct() async {
    final productsAsync = ref.read(sellerProductsProvider((status: 'all', search: null)).future);
    final products = await productsAsync;
    if (!mounted) return;

    final chosen = await showModalBottomSheet<SellerProductSummary>(
      context: context,
                          ...
                          leading: ClipRRect(
                            ...
                            child: SizedBox(
                              ...
                              child: p.imageUrl == null
                                  ...
                                  : Image.network('${resolveApiBaseUrl()}${p.imageUrl}', fit: BoxFit.cover),
                            ),
                          ),
    ...
    if (chosen != null) await _send(productId: chosen.id);
  }
        ...
        title: Row(
          children: [
            CircleAvatar(
              ...
              backgroundImage: widget.room.buyerAvatarUrl == null
                  ? null
                  : NetworkImage('${resolveApiBaseUrl()}${widget.room.buyerAvatarUrl}'),
              child: widget.room.buyerAvatarUrl == null
                  ? const Icon(Icons.person_outline, color: AppColors.primary, size: 18)
                  : null,
            ),
            const SizedBox(width: AppSpacing.sm),
            Text(widget.room.buyerName, style: const TextStyle(fontSize: 16)),
          ],
        ),
      ),
      body: Column(
          ...
          SizedBox(
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, index) => ActionChip(
                ...
                onPressed: _sending ? null : () => _send(body: _quickReplies[index]),
              ...
              child: Padding(
                ...
                child: Row(
                  children: [
                    ...
                    IconButton(
                      onPressed: _sending ? null : _pickProduct,
                      icon: const Icon(Icons.sell_outlined, color: AppColors.textSecondary),
                    ),
                    Expanded(
                      child: TextField(
