...
- [x] Review & rapikan UI/UX semua halaman (konsistensi dengan token desain `shopy-seller`, karena tidak ada mockup acuan)
  - Baru bisa dikerjakan setelah **debug Windows desktop jalan** (butuh workload "Desktop development with C++" di Visual Studio — awalnya `flutter doctor` salah mendeteksi SSMS sebagai VS-nya). Sampai Fase 6, **tidak ada satupun halaman app ini yang pernah benar-benar dirender**; semua verifikasi cuma di level API.
  - `integration_test/screenshot_test.dart` (baru) — membungkus app asli dengan `RepaintBoundary` lalu `toImage()` tiap halaman ke `build/ui_review/*.png` (12 halaman, ukuran surface dipaksa 412×915 karena UI ini mobile-first). Ini yang dipakai buat mereview tampilan, bukan tebak-tebakan dari kode.
  - 🐛 **3 masalah UI ditemukan & diperbaiki** (semuanya lolos dari `flutter analyze`): (1) header login masih **oranye + huruf "S" tiruan** warisan `shopy-seller`, padahal identitas admin itu **navy + logo hexagon asli** (sudah dipakai di splash, app icon, dan indikator bottom nav) — `wave_header.dart` diganti ke `AppColors.secondary` + `SvgPicture.asset('assets/logo.svg')`; (2) kolom pencarian di halaman Cari Produk/Cari Ulasan **ter-center mengikuti tema** sehingga tampak seperti judul kepotong, bukan input — ditambah `centerTitle: false` + ikon `search` sebagai prefix; (3) field form Pengaturan Platform pakai `OutlineInputBorder` polos yang bordernya **hitam tebal**, jomplang dengan field lain di app — disamakan ke border tipis `AppColors.divider` + `filled` seperti `AuthTextField`.
- [ ] Widget test Flutter admin per halaman (pola sama `shopy-seller/test/`) — **sengaja dilewati**, lihat catatan di bawah
- [x] Testing end-to-end (di dokumen awal tertulis "manual", ternyata bisa **otomatis** setelah desktop jalan)
  - `integration_test/admin_flow_test.dart` (baru) — menjalankan app Windows ASLI dan menggerakkan UI-nya beneran: bersihkan storage → login `admin-demo@shopy.com` → keliling 4 tab → buka Cari Produk → buka Pengaturan Platform → logout. Semua lapisan asli (`flutter_secure_storage` nulis ke Credential Manager Windows, `Dio` kirim HTTP sungguhan ke `localhost:5083`).
  - 🐛 **2 bug nyata ditemukan & diperbaiki** — keduanya **mustahil** ketahuan dari `flutter analyze`, `flutter test`, maupun verifikasi curl fase-fase sebelumnya:
    1. **`ListTile` di dalam `Container` ber-`BoxDecoration` berwarna** (tab Lainnya & tab Moderasi) — ripple-nya digambar di `Material` yang posisinya di bawah dekorasi, jadi **setiap tap terasa mati** karena efeknya ketutupan. Diperbaiki dengan `Material(color: Colors.transparent)` + `clipBehavior: Clip.antiAlias`.
    2. **🔴 `Notifier.build()` memanggil `_fetch()` secara sinkron** di **keempat** notifier list (toko, pencairan, produk, ulasan) — `_fetch` menyentuh `state` (`state = state.copyWith(...)`) padahal `state` belum ada sampai `build()` mengembalikan nilai, sehingga Riverpod melempar **"Tried to read the state of an uninitialized provider"**. Bug ini sudah ada **sejak Fase 2** dan lolos terus karena `widget_test.dart` memalsukan `AuthNotifier` sehingga tidak pernah me-mount satupun halaman list. Diperbaiki dengan menunda ke `Future.microtask(...)` di keempatnya.
- [ ] ~~Push notification buat admin~~ — **sengaja dilewati**. Butuh `DeviceToken.AppType` tambah value `Admin` + `NotificationService` method baru + Firebase app ketiga terdaftar; dokumen ini sendiri menandainya "tidak wajib untuk MVP".
- [ ] ~~Seed data demo tambahan~~ — **sengaja dilewati**. Data uji selama ini selalu dibuat lewat skrip verifikasi per fase, dan halaman-halaman admin sudah terbukti memuat data asli (lihat screenshot), jadi tidak ada yang perlu di-seed lagi.
- ⚠️ **Catatan soal widget test (item #2, tidak dikerjakan)**: nilainya tumpang tindih dengan integration test yang sudah ada, tapi jauh lebih lemah — widget test memakai data palsu, sementara justru **dua bug paling parah di atas cuma muncul saat lapisan asli dipakai** (bug #2 malah *disembunyikan* oleh `_FakeAuthNotifier` di `widget_test.dart` yang ada sekarang). Bedanya: integration test butuh backend hidup jadi tidak cocok buat CI; kalau nanti mau test yang selalu hijau tanpa syarat, widget test baru layak ditambahkan.
- ⚠️ **Verifikasi**: `flutter analyze` bersih (3 info lint pra-ada), `flutter test` lulus, `flutter test integration_test/admin_flow_test.dart -d windows` lulus, `screenshot_test.dart` menghasilkan 12 PNG yang sudah direview satu per satu. Prasyarat menjalankan integration test: backend `shopy-api` harus sudah jalan di `localhost:5083` dan akun `admin-demo@shopy.com` ada (dibuat otomatis oleh `AdminSeeder` di environment Development).
